Shader "Custom/Normal/Particle"
{
  Properties
  {
    ...
    _ParticleSize ("粒子大小", Range(0, 0.5)) = 0.01
    ...
    Pass
    {
      ...
      real4 frag(v2f i) : SV_TARGET
      {
        ...
        for (int index = 0; index < _ParticleCount; index++)
        {
          ...
          real time = frac(_Time.y * _ParticleSpeed + timeOffset);

          real2 uv = i.uv - 0.5;
          uv -= _ParticleCenter;
          uv = dir * time - uv;

          real lifetimeSize = 1 - time * _ParticleLifetimeSize;

          real dist = circle(uv, _ParticleSize * lifetimeSize);

          distance = lerp(min(dist, distance), dist, index == 0);
        }
