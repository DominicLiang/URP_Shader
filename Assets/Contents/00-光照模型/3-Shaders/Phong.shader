Shader "Custom/Normal/Phong"
{
  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      half Phong(half3 positionWS, half3 lightDirWS, half3 normalWS, half gloss)
      {
        half3 reflectDir = reflect(-lightDirWS, normalWS);
        half3 viewDirWS = normalize(GetCameraPositionWS() - positionWS);
        half specular = pow(max(0, dot(viewDirWS, reflectDir)), gloss);
        return specular;
      }
      ...
      real4 frag(v2f i) : SV_TARGET
      {
        ...
        real3 color = mainLight.color * NdotL * _MainColor.rgb;

        // Phong
        // real3 reflectDir = reflect(-L, N);
        // real3 V = normalize(GetCameraPositionWS() - i.positionWS);
        // real3 specular = pow(max(0, dot(V, reflectDir)), _Gloss);
        real3 specular = Phong(i.positionWS, L, N, _Gloss);
        specular *= mainLight.color * _SpecularColor.rgb;
        color += specular;
